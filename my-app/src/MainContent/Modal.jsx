        ...
        ${props.modalIsOpen ? 'flex' : 'hidden'} ${props.modalIsOpen ? 'blur-none' : ''}`}>
            ...
            bg-gradient-to-br from-white to-purple4 shadow-md shadow-purple4'>
                <form onSubmit={handleSubmit}>
                    <input name="heading" type="text" className='w-full text-xl font-bold py-3 pl-2 bg-transparent border-dotted border-purple1 border-4 rounded-md' placeholder={props.taskToEdit.name} />
                    <textarea name="description" type="text" className='w-full text-lg font-semibold py-2 pl-2 mt-5 bg-transparent border-dotted border-purple1 border-2 rounded-md'
                        placeholder={props.taskToEdit.description === undefined ? "Description" : props.taskToEdit.description} />
                    
                    <div className='w-full flex flex-row justify-start items-center mt-5 relative'>
                        <select name="tag" className='p-2 bg-transparent  border-double border-purple1 border-4 rounded-md'>
                            <option selected>Uncompeleted</option>
                            <option>In progress</option>
                            <option>Compeleted</option>
                        </select>
                        <input name="color" type="color" className='ml-3 bg-transparent rounded-md'></input>
